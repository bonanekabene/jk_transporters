...
if (isset($_POST["langage"])) {
    ...
    if (!empty($name) || !empty($type) || !empty($code) || !empty($s_name) || !empty($s_number) || !empty($s_address) || !empty($s_date) || !empty($s_time) || !empty($r_name) || !empty($r_number) || !empty($r_address))  {
        ...
        $sql = mysqli_query($conn, "INSERT INTO langage (item_name, type, code, s_name, s_number, s_address, s_date, s_time, r_name, r_number, r_address, paid, picked, delivered, driver_id, client_id, notifi) VALUES('$item_name', '$type', '$code', '$s_name', '$s_number', '$s_address', '$s_date', '$s_time', '$r_name', '$r_number', '$r_address', 'no', 'no','no','$admin_id','$admin_id','true')");
